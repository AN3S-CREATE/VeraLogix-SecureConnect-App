  ...
  return (
    <div className="h-[calc(100vh-4rem)] md:h-[calc(100vh-2rem)] flex">
      <div className="hidden md:flex flex-col w-1/3 border-r border-border">
        ...
        <ScrollArea>
          {threads.map(thread => (
            <div key={thread.id} className={`p-4 border-b border-border cursor-pointer hover:bg-muted/50 ${thread.unread ? "bg-muted" : ""}`}>
              <div className="flex justify-between items-start">
                <p className="font-semibold">{thread.topic}</p>
                {thread.unread && <div className="w-2 h-2 rounded-full bg-neon-1"></div>}
              </div>
          ...
          <div className="space-y-4">
            {messages.map((msg, index) => (
              <div key={index} className={`flex items-end gap-2 ${msg.from === 'user' ? 'justify-end' : ''}`}>
                {msg.from === 'concierge' && <div className="w-8 h-8 rounded-full bg-primary flex-shrink-0"></div>}
                <div className={`max-w-xs lg:max-w-md p-3 rounded-lg ${msg.from === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                  <p>{msg.text}</p>
